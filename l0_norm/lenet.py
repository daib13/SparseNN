...
from layer import dense_l0, dense
...
class lenet300_prune:
    def __init__(self, channel1, channel2):
        self.x = tf.placeholder(tf.float32, [None, 784], name='x')
        self.h1 = dense('layer1', self.x, channel1, activation_fn=tf.nn.relu)
        self.h2 = dense('layer2', self.h1, channel2, activation_fn=tf.nn.relu)
        self.y_logit = dense('layer3', self.h2, 10)
        self.y = tf.placeholder(tf.float32, [None, 10], name='y')

        with tf.name_scope('loss'):
            self.batch_size = tf.cast(tf.shape(self.x, out_type=tf.int32)[0], tf.float32)
            self.oss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=self.y, logits=self.y_logit)) / self.batch_size
        with tf.name_scope('summary'):
            self.summary = tf.summary.scalar('cross_entropy', self.loss)
        with tf.name_scope('accuracy'):
            self.label = tf.arg_max(self.y, -1)
            self.label_hat = tf.arg_max(self.y_logit, -1)
            self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.cast(self.label, tf.int32), tf.cast(self.label_hat, tf.int32)), tf.float32))
